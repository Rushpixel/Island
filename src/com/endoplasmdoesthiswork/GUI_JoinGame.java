...
public class GUI_JoinGame extends GUIRect{
	
	public NumFeild IPFeild;
	public NumFeild PortFeild;
	public NumFeild NameFeild;
	public Button Join;
	public Button Cancel;
	
	public String Error = "";

	public GUI_JoinGame(GUIElement parent) {
		super(parent, new Vertex2f(0, 0), new Vertex2f(300, 300));
		...
		NameFeild = new NumFeild(this, new Vertex2f(5, -35), new Vertex2f(0,0), 8, 12, "Name: ", "PLAYER");
		Join = new Button(this, new Vertex2f(5, -50), new Vertex2f(32, 16), "Join");
		Cancel = new Button(this, new Vertex2f(5, -65), new Vertex2f(40, 16), "Cancel");
		children.add(IPFeild);
		children.add(PortFeild);
		children.add(NameFeild);
		children.add(Join);
		children.add(Cancel);
	}
	...
	@Override
	public void render() {
		Text.renderTextFromString(Error, 5, -83, 8, 12, 50, 2, Endogen.SystemAssets.mask.FONT1, new float[]{1, 0.1f, 0.1f, 1});
	...
	public void update() {
		...
		if(Join.pressed) {
			...
			Client client = new Client(IPFeild.VALUE, port, NameFeild.VALUE);
			Error = client.Begin();
			if(Error.equals("")) {
				GUI.field.children.remove(this);
